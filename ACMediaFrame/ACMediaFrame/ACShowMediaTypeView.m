...
{
    if (!_collectionView) {
        itemW = (ACMedia_ScreenWidth - 5 * space)/5;
        UICollectionViewFlowLayout *layout = [[UICollectionViewFlowLayout alloc]init];
        layout.itemSize = CGSizeMake(itemW, itemW);
        layout.minimumInteritemSpacing = space;
        layout.minimumLineSpacing = space;
        layout.sectionInset = UIEdgeInsetsMake(space/2, space, space/2, space);
        _collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, ACMedia_ScreenHeight, ACMedia_ScreenWidth, itemW * 2 + space * 2) collectionViewLayout:layout];
...
- (instancetype)init {
    ...
    if (self) {
        ...
        self.itemArray = @[@"ACMediaFrame.bundle/Album", @"ACMediaFrame.bundle/Camera", @"ACMediaFrame.bundle/Videotape", @"ACMediaFrame.bundle/Video",@"ACMediaFrame.bundle/codingVideo",@"ACMediaFrame.bundle/videoPlay"];
    }
//    hps12345.
    return self;
}
...
- (void)show {
    ...
    [UIView animateWithDuration:0.3 animations:^{
        self.collectionView.y = ACMedia_ScreenHeight - (itemW * 2 + space * 2);
    }];
}
