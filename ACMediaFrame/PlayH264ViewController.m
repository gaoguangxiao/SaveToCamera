...
@interface PlayH264ViewController ()
{
    uint8_t *_sps;
    NSInteger _spsSize;
    uint8_t *_pps;
...
@implementation PlayH264ViewController
...
-(void)decodeFile:(NSString*)fileName fileExt:(NSString*)fileExt andFilePath:(NSString *)filePath{
    NSString *path;
    if (filePath) {
        path = filePath;
    }else{
        path = [[NSBundle mainBundle] pathForResource:fileName ofType:fileExt];
...
-(IBAction)on_playButton_clicked:(id)sender {
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        [self decodeFile:@"mtv" fileExt:@"h264" andFilePath:self.videoPath];
    });
}
